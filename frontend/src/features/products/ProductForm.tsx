...
const productSchema = z.object({
  ...
  initialStock: z.number().int().min(0, 'El stock inicial no puede ser negativo'),
})
...
function defaults(product?: Product | null, initialCode?: string): ProductFormValues {
  return {
...
export function ProductForm({ product, initialCode, error, onCancel, onSubmit }: ProductFormProps) {
  const {
    register,
    handleSubmit,
    reset,
      ...
      >
        <header className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="product-form-title" className="text-2xl font-semibold tracking-tight">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid gap-5 sm:grid-cols-2">
          ...
          <Field label="Nombre" error={errors.name?.message}>
            <input {...register('name')} className="form-input" placeholder="Cafe molido 500 g" />
          </Field>
