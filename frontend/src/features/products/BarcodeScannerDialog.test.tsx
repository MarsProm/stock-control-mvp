...
describe("BarcodeScannerDialog", () => {
  ...
  it("submits a normalized code from a USB reader", async () => {
    ...
    await user.type(
      screen.getByLabelText("Codigo de barras"),
      " 7791234567890{Enter}",
    );
  ...
  it("blocks duplicate submissions while a lookup is running", async () => {
    ...
    const form = screen
      .getByRole("button", { name: "Buscar codigo" })
      .closest("form");
    ...
    expect(onDetected).toHaveBeenCalledTimes(1);
    expect(
      screen.getByRole("button", { name: "Buscando producto" }),
    ).toBeDisabled();
  });
  ...
  it("offers the manual fallback when camera access is unavailable", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Usar lector USB o entrada manual" }),
    ).toBeInTheDocument();
  });
  ...
  it("stops the camera when the dialog closes", async () => {
    ...
    const { unmount } = render(
      <BarcodeScannerDialog onClose={onClose} onDetected={vi.fn()} />,
    );

    await waitFor(() =>
      expect(
        screen.getByLabelText("Vista previa de la camara"),
      ).toBeInTheDocument(),
    );
