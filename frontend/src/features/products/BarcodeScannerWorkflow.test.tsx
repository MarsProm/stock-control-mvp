...
describe("BarcodeScannerWorkflow", () => {
  it("opens a prefilled product form for an unknown barcode", async () => {
    api.getProductByCode.mockRejectedValueOnce(
      new ApiError(404, { detail: "No existe el producto" }),
    );
    renderWorkflow();
  ...
  it("opens a confirmed stock movement for an existing active product", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Movimiento de Cafe molido" }),
    ).toBeInTheDocument();
  ...
  it("does not allow movements for an inactive product", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Producto inactivo" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Confirmar entrada" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Escanear siguiente" }),
    ).toBeInTheDocument();
  });
});
