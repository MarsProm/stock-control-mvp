...
export function BarcodeScannerDialog({ onClose, onDetected }: BarcodeScannerDialogProps) {
  const dialogRef = useRef<HTMLDialogElement>(null)
  const videoRef = useRef<HTMLVideoElement>(null)
  const manualInputRef = useRef<HTMLInputElement>(null)
  const controlsRef = useRef<IScannerControls | null>(null)
  const processingRef = useRef(false)
  const submitRef = useRef<(rawCode: string) => Promise<void>>(async () => undefined)
  ...
  useEffect(() => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      setCameraError('Este navegador no permite usar la camara. Usa el lector USB o escribe el codigo.')
      return
    }
    ...
    const start = async () => {
      try {
        const controls = await reader.decodeFromConstraints(
          ...
          (result) => {
            if (result && !processingRef.current) void submitRef.current(result.getText())
          },
        )
        if (cancelled) {
          controls.stop()
      ...
      <section className="min-h-dvh bg-white p-5 shadow-2xl sm:min-h-0 sm:rounded-3xl sm:p-7">
        <header className="mb-6 flex items-start justify-between gap-4">
          <div>
            ...
            <h2 id="barcode-scanner-title" className="text-2xl font-semibold tracking-tight">
              Escanear codigo de barras
            </h2>
            <p className="mt-2 text-sm text-slate-600">
              Si el producto es nuevo podras asignarle un nombre y su stock inicial.
            </p>
          </div>
          <button type="button" onClick={onClose} className="icon-button" aria-label="Cerrar escaner">
            <X aria-hidden="true" size={20} />
          </button>
        </header>

        <div role="group" aria-label="Metodo de lectura" className="mb-5 grid grid-cols-2 gap-2 rounded-2xl bg-slate-100 p-1.5">
          <ModeButton active={mode === 'camera'} onClick={() => setMode('camera')} icon={Camera}>
            Camara
          </ModeButton>
          <ModeButton active={mode === 'manual'} onClick={() => setMode('manual')} icon={Keyboard}>
            Lector USB
          </ModeButton>
        </div>
        ...
        {mode === 'camera' ? (
          <div role="tabpanel" className="space-y-4">
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-slate-950">
              <video ref={videoRef} muted playsInline className="size-full object-cover" aria-label="Vista previa de la camara" />
              <div aria-hidden="true" className="pointer-events-none absolute inset-[18%_10%] rounded-2xl border-2 border-emerald-400 shadow-[0_0_0_999px_rgba(2,6,23,0.38)]" />
              {processing ? <ProcessingOverlay /> : null}
            </div>
            <p className="text-center text-sm text-slate-600">Centra el codigo dentro del recuadro y manten el producto quieto.</p>
            {cameraError ? (
              <div role="alert" className="rounded-xl bg-amber-50 p-4 text-sm text-amber-900">
                <p>{cameraError}</p>
                <button type="button" onClick={() => setMode('manual')} className="mt-3 font-semibold underline underline-offset-4">
          ...
          <div role="tabpanel" className="space-y-4">
            <form onSubmit={submitManual} className="space-y-4">
              ...
              <p className="text-sm text-slate-500">Los lectores USB normalmente escriben el codigo y presionan Enter automaticamente.</p>
              <button type="submit" className="button-primary w-full" disabled={processing}>
                {processing ? <LoaderCircle className="animate-spin" aria-hidden="true" size={18} /> : <ScanBarcode aria-hidden="true" size={18} />}
        ...
        <div aria-live="assertive" aria-atomic="true">
          {submissionError ? <p role="alert" className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-700">{submissionError}</p> : null}
        </div>
      </section>
    </dialog>
  )
...
function ProcessingOverlay() {
  return (
    <div className="absolute inset-0 grid place-items-center bg-slate-950/70 text-white" aria-live="polite">
      <div className="flex items-center gap-3 rounded-xl bg-slate-900/90 px-4 py-3 text-sm font-semibold">
        <LoaderCircle className="animate-spin" aria-hidden="true" size={20} /> Buscando producto
      </div>
    </div>
  )
}
