...
describe('ProductForm', () => {
  it('validates required fields before submitting', async () => {
    ...
    expect(await screen.findByText('Usa al menos 3 caracteres')).toBeInTheDocument()
  ...
  it('keeps a scanned code and submits the initial stock', async () => {
    const user = userEvent.setup()
    const onSubmit = vi.fn().mockResolvedValue(undefined)
    render(<ProductForm initialCode="7791234567890" onCancel={vi.fn()} onSubmit={onSubmit} />)
