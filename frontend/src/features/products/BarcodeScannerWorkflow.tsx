...
export function BarcodeScannerWorkflow({
  onClose,
}: BarcodeScannerWorkflowProps) {
  ...
  if (stage.kind === "capture") {
    return (
      <BarcodeScannerDialog onClose={onClose} onDetected={handleDetected} />
    );
  }
    ...
    >
      <section className="rounded-3xl bg-white p-6 text-center shadow-2xl sm:p-8">
        <button
          ...
          aria-label="Cerrar carga rapida"
        >
          <X aria-hidden="true" size={20} />
        </button>
        <span
          className={`mx-auto mb-5 grid size-16 place-items-center rounded-2xl ${iconStyle}`}
        >
